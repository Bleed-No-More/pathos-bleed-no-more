...
void CPlayerMovement::Jump( void )
{
	...
	if(m_pPlayerState->flags & (FL_DEAD|FL_ON_BIKE) || m_pPlayerState->waterlevel == WATERLEVEL_NONE && m_pPlayerState->groundent == NO_ENTITY_INDEX)
	{
		m_pPlayerState->oldbuttons |= IN_JUMP;
		return;
	}
...
void CPlayerMovement::UpdateStepSound( void )
{
	...
	else
	{
		...
		if(speed < PLAYER_NORMAL_SPEED && (m_pPlayerState->flags & FL_SLOWMOVE))
		{
			nextStepTime = STEPTIME_SLOWMOVE;
			volume = 0.5;
		}
		else if(speed < (PLAYER_NORMAL_SPEED - 15))
		{
	...
	// Add in view punch
	if(m_pPlayerState->flags & FL_DUCKING && m_pPlayerState->groundent != NO_ENTITY_INDEX && m_pPlayerState->waterlevel < WATERLEVEL_MID)
	{
		Int32 punchx = 0;
		Int32 punchz = 0;
