...
CGameUIWindow* CGameUIManager::SpawnWindow( gameui_windows_t windowtype )
{
	...
	case GAMEUI_TEXTWINDOW:
			pWindow = CGameUITextWindow::CreateInstance();
		break;
	case GAMEUI_KEYPADWINDOW:
			pWindow = CGameUIKeypadWindow::CreateInstance();
		break;
	...
	if(!pWindow)
	{
		cl_engfuncs.pfnCon_Printf("%s - Window of type %d is not a valid window.\n", __FUNCTION__, (Int32)windowtype);
		return nullptr;
	}

	if(m_pActiveWindow)
	{
		// If we have active windows, put this in the queue
	...
	else
	{
		...
		m_pActiveWindow->setIsInFocus(true);
		...
		if(!m_pBackground)
		{
			...
			m_pBackground = new CGameUISolidSurface(UIEL_FL_NONE, GAMEUIWINDOW_MAIN_TAB_BG_COLOR, 0, 0, screenwidth, screenheight);
		}
	}
...
bool CGameUIManager::Draw( void )
{
	...
	// Enable basic draw
	if(!cl_renderfuncs.pfnEnableBasicDraw() || !cl_renderfuncs.pfnBasicDrawEnableTextures())
		return false;
	...
	modelview.Scale(1.0f/(Float)scrwidth, 1.0f/(Float)scrheight, 1.0);	
	...
	bool result = true;
	if(m_pBackground)
		result = m_pBackground->draw();

	// Draw the window
	if(result)
		result = m_pActiveWindow->draw();
...
// @return Pointer to scheme object
//=============================================
ui_windowdescription_t* CGameUIManager::LoadWindowDescriptionFile( const Char* pstrWindowName, const Char* pstrFilename )
{
...
//=============================================
// @brief Loads in a font set, dependent on screen resolution
//
// @param pstrsetname Font set name
// @return Pointer to font set
//=============================================
const font_set_t* CGameUIManager::GetSchemaResolutionFont( const Char* pstrsetname )
{
	Uint32 screenWidth, sceenHeight;
	cl_renderfuncs.pfnGetScreenSize(screenWidth, sceenHeight);

	Uint32 resolution;
	if(screenWidth > sceenHeight)
		resolution = sceenHeight;
	else
		resolution = screenWidth;

	const font_set_t* pResultSet = cl_engfuncs.pfnGetResolutionSchemaFontSet(pstrsetname, resolution);
	if(!pResultSet)
		pResultSet = m_pFontSet;
